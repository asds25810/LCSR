	...
	for (i = 0; i < n; i++)
	{
		if (flag[i])
		{
			...
			*/
			...
			{
				fout << getPara(file[i][p[i]], "incount") << " ";
				fout << getPara(file[i][p[i]], "request") << " ";
			}
			if (func == "MPI_Test")
			{
			...
			if (func == "MPI_Gather")
			{
				fout << getPara(file[i][p[i]], "sendcnt") << " ";
				fout << getPara(file[i][p[i]], "sendtype") << " ";
				fout << getPara(file[i][p[i]], "recvcount") << " ";
				fout << getPara(file[i][p[i]], "recvtype") << " ";
				fout << getPara(file[i][p[i]], "root") << " ";
			}
			if (func == "MPI_Barrier")
	...
	while ( ! finish(p))
	{
		...
		if (p[0] != file[0].size())
		{
			for (i=1;i<n;i++)
			{
				...
				// The else branch is added by Jingwei Sun. 
				else
				{
					//for debug
					//printf("error: file %d is run out of. %d\n", i, counter);
					same = 0;
					break;
				}
